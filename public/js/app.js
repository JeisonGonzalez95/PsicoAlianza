...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function actualizarHora() {
    var ahora = dayjs().format('HH:mm:ss');
  ...
  var listasToggle = document.getElementById('listasToggle');
  var listasSubmenu = document.getElementById('listasSubmenu');
  ...
  var citySelect = $('#city');
  var selectedCityId = citySelect.data('selected-city');
  function loadCities(stateId) {
    var selectedCityId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    if (stateId) {
  ...
  var selectedPositionId = positionSelect.data('selected-position'); // Si estás pasando el cargo seleccionado con data-selected-position

  function loadPositions(areaId, roleId) {
    var preselectPositionId = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
    if (areaId && roleId) {
      positionSelect.prop('disabled', true).html('<option value="">Cargando...</option>');
      $.ajax({
        url: "/getPositions/".concat(areaId, "/").concat(roleId),
        type: 'GET',
        dataType: 'json',
        success: function success(data) {
          positionSelect.html('<option value="">Seleccione un puesto...</option>');
          $.each(data, function (index, position) {
            var selected = position.id == preselectPositionId ? 'selected' : '';
            positionSelect.append("<option value=\"".concat(position.id, "\" ").concat(selected, ">").concat(position.name_position, "</option>"));
          });
...
document.addEventListener('DOMContentLoaded', function () {
  var alertaElemento = document.getElementById('alerta-data');
  if (alertaElemento) {
    var _datosAlerta$titulo, _datosAlerta$mensaje, _datosAlerta$icono, _datosAlerta$confirma, _datosAlerta$mostrarC, _datosAlerta$cancelar;
    var datosAlerta = JSON.parse(alertaElemento.value);
    Swal.fire({
      title: (_datosAlerta$titulo = datosAlerta.titulo) !== null && _datosAlerta$titulo !== void 0 ? _datosAlerta$titulo : 'Atención',
      text: (_datosAlerta$mensaje = datosAlerta.mensaje) !== null && _datosAlerta$mensaje !== void 0 ? _datosAlerta$mensaje : 'Algo sucedió',
      icon: (_datosAlerta$icono = datosAlerta.icono) !== null && _datosAlerta$icono !== void 0 ? _datosAlerta$icono : 'info',
      confirmButtonText: (_datosAlerta$confirma = datosAlerta.confirmarTexto) !== null && _datosAlerta$confirma !== void 0 ? _datosAlerta$confirma : 'Aceptar',
      showCancelButton: (_datosAlerta$mostrarC = datosAlerta.mostrarCancelar) !== null && _datosAlerta$mostrarC !== void 0 ? _datosAlerta$mostrarC : false,
      cancelButtonText: (_datosAlerta$cancelar = datosAlerta.cancelarTexto) !== null && _datosAlerta$cancelar !== void 0 ? _datosAlerta$cancelar : 'Cancelar'
    });
  }
  var botonesEliminar = document.querySelectorAll('.btn-delete');
